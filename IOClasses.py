...
# Artem Lamnin, 03/26/22, Extended functionality
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        # TODone modified method to accept a list of file names.
        try:
            with open(file_name_CD, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_record())
            # TODone added code to save track data to file
            with open(file_name_track, 'w') as file:
                for row in lst_Inventory:
                    tracks=row.cd_tracks
                    row_id=row.cd_id
                    for line in tracks: 
                        record = '{},{}'.format(row_id, line.get_record()) 
                        file.write(record) 
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            with open(file_name_CD, 'r') as file1:
                for line in file1:
            ...
            with open(file_name_track, 'r') as file2:
                for line in file2:
                    data = line.strip().split(',')
